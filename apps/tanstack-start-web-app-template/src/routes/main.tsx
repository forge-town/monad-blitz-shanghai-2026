...
const Metric = ({
  ...
  return (
    ...
    >
      <div className="font-mono text-[10px] tracking-[0.2em] text-emerald-400/30 uppercase">
        {label}
      </div>
      <div className="mt-0.5 font-mono text-lg font-light tabular-nums text-emerald-300/70">
        {value}
        <span className="text-[10px] text-emerald-400/40">{suffix}</span>
      </div>
    </div>
  );
};
...
const StatusPulse = ({
  ...
  <div className="flex items-center gap-2">
    <div className="relative">
      ...
      />
      {active && (
        <div className="absolute inset-0 h-1.5 w-1.5 animate-ping rounded-full bg-emerald-400/40" />
      )}
    </div>
    <span className="font-mono text-[9px] tracking-wider text-emerald-300/30 uppercase">
      {label}
    </span>
  </div>
);
...
const MainPage = () => {
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-[#0a0f0d]">
      <FlowingGrid />
      <AsciiHero />

      {/* Scan line overlay */}
      <div className="pointer-events-none absolute inset-0 z-5 bg-[repeating-linear-gradient(0deg,transparent,transparent_2px,rgba(16,185,129,0.008)_2px,rgba(16,185,129,0.008)_4px)]" />

      {/* Top-left brand + status */}
      <div className="pointer-events-none absolute left-6 top-5 z-10">
        <div className="flex items-center gap-3">
          <span className="font-mono text-base font-semibold tracking-tight text-emerald-100/50">
            PROVE<span className="text-emerald-400"> IT</span>
          </span>
          <div className="h-3 w-px bg-emerald-500/20" />
          <span className="font-mono text-[9px] tracking-[0.15em] text-emerald-400/25">
            MONAD TESTNET
          </span>
        </div>
      ...
      <div className="pointer-events-none absolute bottom-24 left-6 z-10 flex flex-col gap-3">
        <div className="font-mono text-[8px] tracking-[0.3em] text-emerald-500/20 uppercase">
          Live Feed
        </div>
        <LiveFeed />
      </div>
      ...
      <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center">
        <div className="text-center">
          <h1 className="font-mono text-[clamp(2rem,5vw,4rem)] font-extralight tracking-[-0.02em] text-emerald-100/[0.07]">
            PROVE IT
          </h1>
          <p className="mt-2 font-mono text-[11px] tracking-[0.25em] text-emerald-400/20 uppercase">
      ...
      <Link
        to="/arena"
        className="group absolute bottom-6 right-6 z-10 flex items-center gap-2 border border-emerald-500/20 bg-emerald-950/30 px-6 py-2.5 font-mono text-[11px] font-medium text-emerald-300/50 backdrop-blur-sm transition-all hover:border-emerald-400/50 hover:bg-emerald-900/20 hover:text-emerald-300 hover:shadow-[0_0_20px_rgba(16,185,129,0.1)]"
      >
        ENTER ARENA
  ...
  return (
    <div className="flex flex-col gap-1">
      {items.map((item, i) => (
        <div
          key={`${item}-${i}`}
          className="max-w-65 truncate font-mono text-[8px] text-emerald-400/20 transition-opacity duration-500"
          style={{ opacity: 1 - i * 0.2 }}
        >
          <span className="text-emerald-500/30">›</span> {item}
        </div>
      ))}
    </div>
  );
...
const BlockTicker = () => {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="h-1 w-1 animate-pulse rounded-full bg-emerald-500/40" />
      <span className="font-mono text-[9px] tabular-nums text-emerald-500/25">
