...
const ArenaPage = () => {
  ...
  const [steps, setSteps] = useState<PipelineStep[]>([
    { id: "register", label: "Register Agents", detail: "Stake 0.01 MON × 3 agents", status: "idle" },
    { id: "task", label: "Create Task", detail: "Reward pool: 0.02 MON", status: "idle" },
    { id: "solve", label: "AI Solve", detail: "3 agents via Claude API", status: "idle" },
    { id: "commit", label: "Commit Hashes", detail: "Monad parallel EVM", status: "idle" },
    { id: "reveal", label: "Reveal Results", detail: "Public verification", status: "idle" },
    { id: "judge", label: "Judge & Settle", detail: "Consensus → rewards", status: "idle" },
  ]);
  ...
  const handleCreateTask = async () => {
    ...
    try {
      ...
      createTask(
        TASK.description,
        TASK.taskType,
        parseEther(TASK.requiredStakeEth),
        now + 600n,
        now + 1200n,
        5,
        parseEther(TASK.rewardEth),
      );
      addLog(`  → Task: "${TASK.description.slice(0, 50)}..."`);
      addLog(`  → Reward: ${TASK.rewardEth} MON`);
      updateStep("task", { status: "done", result: `${TASK.rewardEth} MON reward pool` });
  ...
  const handleSolve = async () => {
    ...
    try {
      ...
      const results: AgentResult[] = result.results.map((r) => ({
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      for (const r of agentResults) {
        ...
        addLog(`  → ${r.agentName} hash committed`);
      }
  ...
  const handleJudge = async () => {
    ...
    try {
      ...
      addLog(`  → Consensus: [${judgment.consensus.map((i) => agentResults[i]?.agentName).join(", ")}]`);
      if (judgment.outliers.length > 0) {
        addLog(`  → Outliers: [${judgment.outliers.map((i) => agentResults[i]?.agentName).join(", ")}]`);
      }
      addLog(`  → ${judgment.reasoning}`);

      const currentTaskId = taskCount ? BigInt(Number(taskCount) - 1) : 0n;
      startJudging(currentTaskId);
      const consensusAgents = judgment.consensus.map(
      ...
      updateStep("judge", {
        status: "done",
        result: `${judgment.consensus.length} consensus / ${judgment.outliers.length} slashed`,
      });
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[#f4f9f6] text-zinc-900">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-teal-200/40 px-4 py-2">
        <div className="flex items-center gap-3">
          <Link to="/main" className="text-zinc-400 hover:text-teal-600">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="flex w-[260px] shrink-0 flex-col border-r border-teal-200/40">
          ...
          <div className="border-t border-teal-200/40 bg-white/40 px-3 py-2">
            ...
            <p className="mt-0.5 line-clamp-2 font-mono text-[10px] text-zinc-600">
              {TASK.description}
            </p>
            <div className="mt-1 flex gap-3 font-mono text-[9px] text-zinc-400">
              <span>{TASK.rewardEth} MON</span>
              <span>×3 agents</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="flex shrink-0 flex-col border-b border-teal-200/40">
            ...
            <div className="grid min-h-[180px] grid-cols-3 gap-px bg-teal-100/30">
              {agentResults.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      <div className="mb-1.5 flex items-center justify-between">
                        <span className="font-mono text-[10px] font-semibold text-zinc-700">{r.agentName}</span>
                        {isConsensus && (
                          <span className="border border-teal-500 px-1 py-px font-mono text-[7px] font-bold uppercase text-teal-700">
                            Valid
                          </span>
                        )}
                        {isOutlier && (
                          <span className="border border-red-400 px-1 py-px font-mono text-[7px] font-bold uppercase text-red-600">
                            Slash
                          </span>
                        )}
                      </div>
                      <p className="flex-1 overflow-hidden font-mono text-[10px] leading-relaxed text-zinc-500">
                        {r.answer}
                      </p>
                    </div>
                  );
                ...
                <>
                  <div className="flex items-center justify-center bg-[#f4f9f6] p-3">
                    <span className="font-mono text-[10px] text-zinc-300">Agent 1</span>
                  </div>
                  <div className="flex items-center justify-center bg-[#f4f9f6] p-3">
                    <span className="font-mono text-[10px] text-zinc-300">Agent 2</span>
                  </div>
                  <div className="flex items-center justify-center bg-[#f4f9f6] p-3">
                    <span className="font-mono text-[10px] text-zinc-300">Agent 3</span>
                  </div>
                </>
              )}
            </div>
          ...
          {/* Terminal (bottom half, fills remaining) */}
